...
for col in final_headers:
    ...
    print("\n=== ROW INDEXES ===")
    row_map = {}
    for i, idx in enumerate(range(2, len(df)), start=1):
        row_map[str(i)] = idx
        print(f"{i}: {df.loc[idx, 'Nutriente']}")

    print("\nType the row numbers (space-separated) that have blanks. Add 'R' at the end to fill the rest from the last row onward.")
    raw_input = input("Rows with blanks: ").strip().split()

    selected_row_nums = []
    fill_remaining_from = None

    for val in raw_input:
        if val.upper() == "R":
            if selected_row_nums:
                last_row = max(int(n) for n in selected_row_nums)
                fill_remaining_from = last_row + 1
            else:
                fill_remaining_from = 1  # From the start if no rows were given
        elif val.isdigit() and val in row_map:
            selected_row_nums.append(val)
        else:
            print(f"⚠️ Invalid row number: {val}")

    remaining_blanks = total_blanks

    # Fill specified rows
    for row_str in selected_row_nums:
    ...
    # Fill remaining rows starting from `fill_remaining_from`
    if fill_remaining_from is not None and remaining_blanks > 0:
        for i in range(fill_remaining_from, len(row_map) + 1):
            if remaining_blanks <= 0:
                break
            idx = row_map[str(i)]
            val_index = df.columns.get_loc(col)
