...
# === ASK FOR BLANKS & MODIFY DATAFRAME (Handles trailing R for remaining rows) ===
...
for col in final_headers:
    ...
    for idx in range(2, len(df)):
        raw_title = df.loc[idx, 'Nutriente']
        display_title = raw_title.replace("_", " ")
        # print(display_title)

        normalized_key = display_title.upper()
        title_to_idx[normalized_key] = idx
        normalized_to_actual[normalized_key] = raw_title

    response = input(f"\nBlanks for column '{col}'? (A = all, 0 = zero, or row titles like 'Humedad, Proteínas' or 'Humedad - Proteínas, R'): ").strip().upper()
    ...
    raw_input = [x.strip() for x in response.split(",")]
    selected_keys = set()
    fill_remaining_from_idx = None

    for part in raw_input:
        if part == "R":
            if selected_keys:
                last_idx = max(title_to_idx[k] for k in selected_keys)
                fill_remaining_from_idx = last_idx + 1
            else:
                fill_remaining_from_idx = 2
        elif " - " in part:
            try:
                start_text, end_text = map(str.strip, part.split(" - "))
                start_key = start_text.upper()
                end_key = end_text.upper()
                if start_key in title_to_idx and end_key in title_to_idx:
                    start_idx = title_to_idx[start_key]
                    end_idx = title_to_idx[end_key]
                    for idx in range(start_idx, end_idx + 1):
                        title = df.loc[idx, 'Nutriente']
                        selected_keys.add(title.upper().replace("_", " "))
                else:
            ...
            if key in title_to_idx:
                ...
                print(f"⚠️ Invalid title: {part}")

    # Apply blanks to selected rows
    for key in sorted(selected_keys, key=lambda k: title_to_idx[k]):
    ...
    # Fill remaining from R
    if fill_remaining_from_idx is not None:
        for idx in range(fill_remaining_from_idx, len(df)):
            val_index = df.columns.get_loc(col)
            current_val = df.iat[idx, val_index]
            if pd.isna(current_val) or current_val == "":
                df.iat[idx, val_index] = "-"
